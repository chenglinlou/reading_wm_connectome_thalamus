load("network_pre_nos.mat", "allh",'all_thres','all_bin')
load('bh.mat')
load('sub.mat')
...
aaa = [cc_thala,Eloc_thala,bh,fa_subnet];

[cc, cp] = partialcorr(aaa(:,1:7),aaa(:,[8:10]))
% p = polyfit(aaa(:,1),aaa(:,7),1);
% yfit = polyval(p,aaa(:,1));
% plot(aaa(:,1),aaa(:,7),'bo',aaa(:,1),yfit,'k-');
